...
describe('QuestionService', () => {
  ...
  beforeEach(async () => {
    ...
    characteristicModel = module.get<Model<Characteristic>>(getModelToken(Characteristic.name));
  ...
  describe('create', () => {
    it('should create a question with characteristics', async () => {
      ...
      jest.spyOn(characteristicModel, 'find').mockResolvedValue(mockCharacteristics as any);
      jest.spyOn(questionModel, 'constructor' as any).mockReturnValue(savedQuestion);

      const result = await service.create(mockDto as any);

      expect(characteristicModel.find).toHaveBeenCalledWith({
        _id: {
          $in: mockDto.characteristics.map((c) => new Types.ObjectId(c.characteristicId)),
        },
      });

      expect(result.characteristics).toHaveLength(2);
      expect(logger.info).toHaveBeenCalledWith('Created question:', expect.any(Object));
    });
    ...
    it('should throw an error if characteristics are not found', async () => {
      ...
      await expect(service.create(mockDto as any)).rejects.toThrow(HttpException);
